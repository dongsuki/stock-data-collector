...
def get_moving_averages(symbol):
    ...
    try:
        # 기술적 지표 데이터 요청
        rate_limiter.wait_if_needed()
        url = f"https://financialmodelingprep.com/api/v3/technical-indicators/{symbol}?period=10&type=sma&apikey={FMP_API_KEY}"
        response = requests.get(url, timeout=30)
        ...
        if response.status_code == 200:
            data = response.json()
            if data:
                latest_data = data[0]  # 최신 데이터
                ma_data['MA50'] = safe_float(latest_data.get('SMA50'))
                ma_data['MA150'] = safe_float(latest_data.get('SMA150'))
                ma_data['MA200'] = safe_float(latest_data.get('SMA200'))
                
                # 200일 이평선 추세 확인
                if len(data) >= 30:  # 한 달치 데이터가 있는지 확인
                    month_ago_ma200 = safe_float(data[20].get('SMA200'))
                    if month_ago_ma200 > 0:
                        ma_data['MA200_trend'] = ma_data['MA200'] > month_ago_ma200

    except Exception as e:
        print(f"❌ {symbol} 이평선 데이터 가져오기 실패: {str(e)}")

    # 모든 필요한 데이터가 있는지 확인
    if not all(ma_data[key] is not None and ma_data[key] > 0 for key in ['MA50', 'MA150', 'MA200']):
