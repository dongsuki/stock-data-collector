...
def calculate_traded_value(stocks):
    """거래대금 계산 및 정렬"""
    for stock in stocks:
    ...
    # 거래대금을 기준으로 내림차순 정렬 후 상위 20개 선택
    top_20 = sorted(stocks, key=lambda x: x.get('traded_value', 0), reverse=True)[:20]
    return top_20
...
def main():
    ...
    top_20_traded_value = calculate_traded_value(all_stocks)
    print(f"\n거래대금 상위 20개 종목 선정 완료")
    
    if top_20_traded_value:
        update_airtable(top_20_traded_value, "거래대금 상위")
